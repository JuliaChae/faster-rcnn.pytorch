...
def loss(): 
   ...
   for line in lines[1:]:
      train[0][i] = i +1
      train[1][i] = float(line.split("    ")[2])
      if float(line.split("    ")[3][:-2]) != 0.:
         val[0][((i+1)//5)-1] = i +1
         val[1][((i+1)//5)-1] = float(line.split("    ")[3])
      i = i +1 
...
def train_AP():
   ...
   lines = f.readlines()
   train = np.zeros([11,97])

   i=0
   for line in lines[1:]:
      train[0][i] = i + 1
      for x in range(1,11):
         train[x][i] = float(line.split("    ")[x-1])
      i = i +1 
